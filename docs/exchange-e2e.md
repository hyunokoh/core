...
- Binance-compatible private `/v3/order` same-account crossing asks against an own resting bid are rejected by self-trade prevention with `direction=ASK`, leave the resting bid intact, create no self-trade row, and release the rejected ask's base reservation.
- Binance-compatible private `/v3/order` same-account `MARKET` sells against an own resting bid are rejected by self-trade prevention with `direction=ASK`, leave the resting bid intact, create no self-trade row, and release the rejected market ask's base reservation.
- Binance-compatible private `/v3/order` same-account price-capped `MARKET` buys against an own resting ask are rejected by self-trade prevention with `direction=BID`, leave the resting ask intact, create no self-trade row, and release the rejected market bid's quote reservation.
- Binance-compatible `/v3/account` returns integer-encoded `makerCommission`/`takerCommission` matching the configured fee schedule, with `buyerCommission`/`sellerCommission` zero, `accountType=SPOT`, all permissions enabled, and SPOT in the permissions list.
- Binance-compatible `/v3/depth` returns a positive integer `lastUpdateId`, well-formed `[price, quantity]` rows on both sides, and aggregates multiple same-price open orders from different owners into a single depth level.
- Binance-compatible `/v3/exchangeInfo` returns a populated `timezone`, positive `serverTime`, array-typed `rateLimits` and `exchangeFilters`, and per-symbol `baseAssetPrecision`/`quoteAssetPrecision`, boolean `icebergAllowed`/`ocoAllowed`, an `orderTypes` array, and a `permissions` list containing `SPOT`.
- Binance-compatible `/v3/ticker/24h` returns non-negative `bidPrice`, `askPrice`, `firstId`, and `lastId` with `lastId >= firstId` so order book snapshot fields and trade ID metadata are not silently zero or stale.
- Binance-compatible `/v3/myTrades` rows carry string `symbol`/`commissionAsset` and a positive numeric epoch-ms `time`, and `/v3/allOrders` rows carry positive numeric `time`/`updateTime` with `updateTime >= time`, matching Binance spec timestamps.
...
- Binance-compatible wallet history endpoints reject stale signed request windows before reading deposit or withdraw history.
...
- Binance-compatible private trade fee endpoints return the accountant-configured maker/taker fee schedule for a requested symbol and for all symbols.
...
- Binance API STP-ask owner: `1 ETH` and `100 USDT` after a same-account `/v3/order` crossing ask is rejected without creating a trade, then the resting bid is canceled.
- Binance API STP-market owner: `1 ETH` and `100 USDT` after a same-account `/v3/order` MARKET sell against the owner's resting bid is rejected without creating a trade, then the resting bid is canceled.
- Binance API STP-market-buy owner: `1 ETH` and `100 USDT` after a same-account `/v3/order` price-capped MARKET buy against the owner's resting ask is rejected without creating a trade, then the resting ask is canceled.
- Binance API underfunded owner: `0.5 ETH` and `50 USDT` after underfunded `/v3/order` ask and bid requests are rejected with no open or historical order projection.
...
- Final database state has zero E2E `EXCHANGE` wallet balances, wallet transaction counts of `90 DEPOSIT`, `85 ORDER_CREATE`, `53 ORDER_CANCEL`, `46 TRADE`, `46 FEE`, and `1 ORDER_FINALIZED`, processed Accountant financial action counts of `85 SubmitOrderEvent`, `44 CancelOrderEvent`, `6 RejectOrderEvent`, `93 TradeEvent`, and `3 UpdatedOrderEvent`, no pending/given-up Accountant retries, zero Market `open_orders`, internally consistent Market trade rows, and the same persisted trade distribution as the public recent-trades API.
...
- The script waits for accountant, matching-engine, matching-engine-duo, and market Kafka assignments before submitting orders.
