...
enum class OpexError(val code: Int, val message: String?, val status: HttpStatus) : ErrorRep {
    ...
    ServiceUnavailable(1006, "Service unavailable", HttpStatus.SERVICE_UNAVAILABLE),
    ...
    SubmitOrderForbiddenByAccountant(4001, "Order was rejected by accountant", HttpStatus.BAD_REQUEST),
    ...
    // code 6000: wallet
    WalletOwnerNotFound(6001, "Wallet owner not found", HttpStatus.NOT_FOUND),
    WalletNotFound(6002, "Wallet not found", HttpStatus.NOT_FOUND),
    CurrencyNotFound(6003, "Currency not found", HttpStatus.NOT_FOUND),
    ...
    CurrencyIsExist(6007, "Currency already exists", HttpStatus.BAD_REQUEST),
    PairIsExist(6008, "Pair already exists", HttpStatus.BAD_REQUEST),
    ForbiddenPair(6009, "Pair is forbidden", HttpStatus.BAD_REQUEST),
    InvalidRate(6010, "Invalid rate", HttpStatus.BAD_REQUEST),
    PairNotFound(6011, "Pair not found", HttpStatus.BAD_REQUEST),
    SourceIsEqualDest(6012, "Source and destination cannot be the same", HttpStatus.BAD_REQUEST),
    AtLeastNeedOneTransitiveSymbol(6013, "At least one transitive symbol is required", HttpStatus.BAD_REQUEST),
    CurrencyIsDisable(6014, "Currency is disabled", HttpStatus.BAD_REQUEST),
    CurrencyIsTransitiveAndDisablingIsImpossible(6015, "Transitive currency cannot be disabled", HttpStatus.BAD_REQUEST),
    InvalidReserveNumber(6016, "Invalid reserve number", HttpStatus.BAD_REQUEST),
    CurrentSystemAssetsAreNotEnough(6017, "Current system assets are not enough", HttpStatus.INTERNAL_SERVER_ERROR),
    NotEnoughBalance(6018, "Not enough balance", HttpStatus.BAD_REQUEST),
    WithdrawNotAllowed(6019, "Withdraw is not allowed", HttpStatus.BAD_REQUEST),
